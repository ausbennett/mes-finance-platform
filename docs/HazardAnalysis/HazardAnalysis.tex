...
\midrule
  ...
  1/11/2025 & Austin Bennett & System Components, notification and invoice modules revisions.\\
...
\begin{itemize}
  ...
  \item \textbf{Invoice Generation Module:} A partition of the system that allows users to generate and submit personalized invoices. For instance, users can create detailed invoices for reimbursement requests, ensuring compliance with required financial documentation formats and aiding in seamless transaction records.
  \item \textbf{Notification Module:} A partition of the system responsible for sending automated notifications to users. For example, it alerts users when their payment or reimbursement requests are approved, provides reminders for pending actions, and informs users of system updates via email to ensure effective communication. 
...
\parbox[t]{5cm}{\raggedright
    \begin{enumerate}
      \item[a.] Weak password policies/restrictions allowing use of simple/commonly used passwords
      \item[b.] Lack of multi-factor authentication
    \end{enumerate}
  } &
\parbox[t]{5cm}{\raggedright
    \begin{enumerate}
        \item[a.] Enforce users to create strong passwords (over 8 characters, including special symbols and upper/lower cases)
        \item[b.] Implement and enforce multi-factor authentication of at least 2 steps (eg. Microsoft Authenticator)
    \end{enumerate}
} &
...
%Row 2
&
\parbox[t]{3cm}{\raggedright User accounts are compromised/leaked to a third party} &
\parbox[t]{3cm}{\raggedright Data theft, manipulation, or system sabotage} &
\parbox[t]{5cm}{\raggedright
    \begin{enumerate}
      \item[a.] Weak or reused passwords, including commonly-used passwords on the internet
    \end{enumerate}
  } &
\parbox[t]{5cm}{\raggedright
    \begin{enumerate}
      \item[a.] Enforce strong password policies, as above
      \item[b.] Encrypt sensitive password information 
    \end{enumerate}
  } &
\parbox[t]{2cm}{\raggedright
    \begin{enumerate}
...
%Row 3
&
\parbox[t]{3cm}{\raggedright Unauthorized privilege escalation by users} &
\parbox[t]{3cm}{\raggedright Users perform actions above their authorization level} &
\parbox[t]{5cm}{\raggedright
    \begin{enumerate}
...
\parbox[t]{5cm}{\raggedright
    \begin{enumerate}
      \item[a.] Use role-based access control, including checks before certain actions are performed
    \end{enumerate}
  } &
\parbox[t]{2cm}{\raggedright
    \begin{enumerate}
...
\parbox[t]{5cm}{\raggedright
    \begin{enumerate}
      \item[a.] Poor security practices for accessing and storing finance info
      \item[b.] Lack of encryption of data in transit 
    \end{enumerate}
  } &
\parbox[t]{5cm}{\raggedright
    \begin{enumerate}
        \item[a.] Encrypt all data containing sensitive financial information at rest and in transit, including masking card numbers with * symbol
    \end{enumerate}
} &
...
%Row 1
\parbox[t]{2.5cm}{\raggedright Data Backup and Recovery} & \parbox[t]{3cm}{\raggedright Scheduled data backup fails to execute or complete} & \parbox[t]{3cm}{\raggedright Financial data is lost, compromising transaction history and further transactions} &
\parbox[t]{5cm}{\raggedright
    \begin{enumerate}
...
\parbox[t]{5cm}{\raggedright
    \begin{enumerate}
      \item[a.] Remote database failures or propagation errors when transmitting data to the backup database
    \end{enumerate}
  } &
\parbox[t]{5cm}{\raggedright
    \begin{enumerate}
      \item[a.] Store backed-up data in multiple, secure locations
      \item[b.] Verify data was not modified after being backed up
    \end{enumerate}
  } &
\parbox[t]{2cm}{\raggedright
    \begin{enumerate}
