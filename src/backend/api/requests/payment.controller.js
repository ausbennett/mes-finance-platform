...
const createPayment = async (req, res) => {
  ...
  if (payment.message) {
    return res.status(400).json(payment); // error message
  }

  return res.status(201).json(payment); // success, created
};

// PUT - Edits an existing payment
const editPayment = async (req, res) => {
  ...
  return res.status(200).json(payment); // success, updated
};
